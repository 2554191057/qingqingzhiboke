...
- 公网入口为 `yanzheng.html`（图形验证码，验证通过跳 boke.html；背景已改为樱花引导页 NianBroken/Personal_Sakura_Guide_Page 同款 WebGL 樱花飘落：深色 #1b1f22 背景 + 全屏 #sakura canvas，脚本在 `js/sakura/`，按 config→math→shaders→gl-utils→renderer→bootstrap 顺序引入）；主域名 `https://qqzttkx.ficp.fun/` 仍绑定旧资源 `hxa7wt`（2026-09-18 窄屏修复前部署，旧内容待切换）；**最新部署资源 `h5im6n`（resource_id 1789674464887216645，临时 https://h5im6n.gicp.fun，2026-09-18 复制IP功能完成部署：聊天室后台恢复 escHtml/enrichIps/bindLogToggle/handleSetLogCat 定义+新增复制IP点击监听，访客消息/黑名单IP也加复制按钮，含此前 visit 页面事件类型与窄屏修复等全部历史；此前 9missw(1789673925894477725) visit页面事件类型部署、whbe3i(1789673155477040451) 窄屏修复+vore.top区县部署，主域名切换见资源链）**。Cloudflare 镜像 `qqzttkx.eu.cc` 与 GitHub Pages 由 git push 自动同步（2026-09-18 实测已更新，本次部署 https://6fb02516.qingqingzhiboke.pages.dev）。部署后立即 curl 验证时可能因 CDN 分发显示短内容/404，需稍候重试；curl 长输出用 `-o 文件` 方式读，避免 PowerShell 管道截断误报。
