...
- 公网入口为 `yanzheng.html`（图形验证码，验证通过跳 boke.html；背景已改为樱花引导页 NianBroken/Personal_Sakura_Guide_Page 同款 WebGL 樱花飘落：深色 #1b1f22 背景 + 全屏 #sakura canvas，脚本在 `js/sakura/`，按 config→math→shaders→gl-utils→renderer→bootstrap 顺序引入）；主域名 `https://qqzttkx.ficp.fun/` 当前待绑定资源 `rhqp8r`（resource_id 1789563504357075617，临时 https://rhqp8r.gicp.fun，2026-09-16 导航改名部署；临时域名已验证含"文章资源/网盘分享"新文案、style.css?v=267；主域名仍指旧资源 0z9rsw，需用户在控制台删旧绑新）。部署后立即 curl 验证时可能因 CDN 分发显示短内容/404，需稍候重试；curl 长输出用 `-o 文件` 方式读，避免 PowerShell 管道截断误报。
