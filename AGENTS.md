...
- 公网入口为 `yanzheng.html`（图形验证码，验证通过跳 boke.html；背景已改为樱花引导页 NianBroken/Personal_Sakura_Guide_Page 同款 WebGL 樱花飘落：深色 #1b1f22 背景 + 全屏 #sakura canvas，脚本在 `js/sakura/`，按 config→math→shaders→gl-utils→renderer→bootstrap 顺序引入）；主域名 `https://qqzttkx.ficp.fun/` 当前绑定资源 `scj6yt`（resource_id 1789553652545030793；临时域名 https://scj6yt.gicp.fun，2026-09-16 部署，已线上验证 fklts 含验证拦截+悬浮组件、yanzheng/boke 200）。
