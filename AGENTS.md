...
- 公网入口为 `yanzheng.html`（图形验证码，验证通过跳 boke.html；背景已改为樱花引导页 NianBroken/Personal_Sakura_Guide_Page 同款 WebGL 樱花飘落：深色 #1b1f22 背景 + 全屏 #sakura canvas，脚本在 `js/sakura/`，按 config→math→shaders→gl-utils→renderer→bootstrap 顺序引入）；主域名 `https://qqzttkx.ficp.fun/` 仍绑定旧资源 `hxa7wt`（2026-09-18 窄屏修复前部署，旧内容待切换）；**最新部署资源 `riq9gj`（resource_id 1789693097500127875，临时 https://riq9gj.gicp.fun，2026-09-18 聊天室修复部署：恢复 3ba7f83 误删的 25 个函数（doLogin/refreshLoginUI/openLogin/logout/账号设置/回复/点赞等）+ 顶层按钮绑定（管理/退出/设置tab/改昵称/改密码/改邮箱/发验证码）+ 常量（LK/QW_NICK_KEY/QW_NICK/QW_EMAIL/DK/loginMode），修复手机端聊天室不显示聊天框/信息、关闭/管理按钮点不了；此前 iq5n66(1789692415162841464) 日志多选删除部署、k7flxm(1789692085980926686) 事件类型中文化+区县修复部署，主域名切换见资源链）**。Cloudflare 镜像 `qqzttkx.eu.cc` 与 GitHub Pages 由 git push 自动同步（2026-09-18 实测已更新，本次部署 https://6fb02516.qingqingzhiboke.pages.dev）。部署后立即 curl 验证时可能因 CDN 分发显示短内容/404，需稍候重试；curl 长输出用 `-o 文件` 方式读，避免 PowerShell 管道截断误报。
