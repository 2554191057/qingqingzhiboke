...
## 在线部署（每次修改后必须执行）
本项目使用 HSK 静态文件托管。**当前有效资源**：`1789472499149792180`（2026-09-15 创建，资源访问地址 https://mvwyed.gicp.fun）。
**公网域名 `https://qqzttkx.ficp.fun/`** 目前仍绑定在资源 `1789471342039255230`（该资源已被平台禁用更新、内容冻结在 style v=265）；如需域名指向最新内容，需在花生壳后台把域名重新绑定到当前有效资源 `1789472499149792180`（**尚未绑定，绑定后须在 AGENTS.md 更新说明**）。
旧资源 `1787743956892235141` / `1789471342039255230` / `1789471753168216449` 均已被平台禁用更新（403 update function is disabled），不得再用于部署。

**每次修改 index.html / style.css / script.js（或任意网站文件）并 git 提交后，必须同步执行一次在线部署：**

```bash
hsk-cli file-hosting "D:\Download\qingqingzhiboke" --entry-file index.html --resource-id 1789472499149792180 --format json
```
...
- 部署成功后用 `curl https://mvwyed.gicp.fun/` 验证线上版本（index.html 里的 style.css/script.js 版本号）与本地一致。
- **严禁**再新建别的 HSK 资源来部署本网站（会占用新资源）；一律复用资源 `1789472499149792180`。
