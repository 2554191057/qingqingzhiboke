...
- 公网入口为 `yanzheng.html`（图形验证码，验证通过跳 boke.html；背景已改为樱花引导页 NianBroken/Personal_Sakura_Guide_Page 同款 WebGL 樱花飘落：深色 #1b1f22 背景 + 全屏 #sakura canvas，脚本在 `js/sakura/`，按 config→math→shaders→gl-utils→renderer→bootstrap 顺序引入）；主域名 `https://qqzttkx.ficp.fun/` 仍绑定旧资源 `hxa7wt`（2026-09-18 窄屏修复前部署，旧内容待切换）；**最新部署资源 `z9kdqu`（resource_id 1789693678278208929，临时 https://z9kdqu.gicp.fun，2026-09-18 登录弹窗头像+密码规则部署：登录弹窗顶部圆形图标改为默认用户头像（person SVG）；密码规则升级为至少8位且不能纯数字（需数字+字母或符号，新增 qwPwdOk 校验，覆盖注册/改密码前端校验与提示文案；后端 twikoo.js 同步 pwdStrong 校验于注册/改密码/管理员重置，commit 9e516f3 已 netlify prod 部署）；此前 riq9gj(1789693097500127875) 聊天室修复部署（恢复误删25函数+绑定+常量）、iq5n66(1789692415162841464) 日志多选删除部署，主域名切换见资源链）**。Cloudflare 镜像 `qqzttkx.eu.cc` 与 GitHub Pages 由 git push 自动同步（2026-09-18 实测已更新，本次部署 https://6fb02516.qingqingzhiboke.pages.dev）。部署后立即 curl 验证时可能因 CDN 分发显示短内容/404，需稍候重试；curl 长输出用 `-o 文件` 方式读，避免 PowerShell 管道截断误报。
