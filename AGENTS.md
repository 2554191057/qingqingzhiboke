...
- 公网入口为 `yanzheng.html`（图形验证码，验证通过跳 boke.html；背景已改为樱花引导页 NianBroken/Personal_Sakura_Guide_Page 同款 WebGL 樱花飘落：深色 #1b1f22 背景 + 全屏 #sakura canvas，脚本在 `js/sakura/`，按 config→math→shaders→gl-utils→renderer→bootstrap 顺序引入）；主域名 `https://qqzttkx.ficp.fun/` 仍绑定旧资源 `hxa7wt`（2026-09-18 窄屏修复前部署，旧内容待切换）；**最新部署资源 `k7flxm`（resource_id 1789692085980926686，临时 https://k7flxm.gicp.fun，2026-09-18 事件类型中文化部署：visit_*页面事件日志显示为'访问首页/文章/资源/聊天室'等中文+IP归属地补区县（vore.top过滤基站占位、无区县时ipwho.is补充district、回退路径加district、缓存_v4）；此前 qxnx3c(1789691922162287749) 区县修复部署、h5im6n(1789674464887216645) 复制IP功能部署，主域名切换见资源链）**。Cloudflare 镜像 `qqzttkx.eu.cc` 与 GitHub Pages 由 git push 自动同步（2026-09-18 实测已更新，本次部署 https://6fb02516.qingqingzhiboke.pages.dev）。部署后立即 curl 验证时可能因 CDN 分发显示短内容/404，需稍候重试；curl 长输出用 `-o 文件` 方式读，避免 PowerShell 管道截断误报。
