...
- 公网入口为 `yanzheng.html`（图形验证码，验证通过跳 boke.html；背景已改为樱花引导页 NianBroken/Personal_Sakura_Guide_Page 同款 WebGL 樱花飘落：深色 #1b1f22 背景 + 全屏 #sakura canvas，脚本在 `js/sakura/`，按 config→math→shaders→gl-utils→renderer→bootstrap 顺序引入）；主域名 `https://qqzttkx.ficp.fun/` 仍绑定旧资源 `hb8llj`（临时 https://hb8llj.gicp.fun，旧内容，待用户控制台手动切换）；**最新部署资源 `hxa7wt`（resource_id 1789671297244903532，临时 https://hxa7wt.gicp.fun，2026-09-18 验证页本机IP展示+复制IP按钮部署；主域名切换见资源链）**。Cloudflare 镜像 `qqzttkx.eu.cc` 与 GitHub Pages 由 git push 自动同步（2026-09-18 实测已更新）。部署后立即 curl 验证时可能因 CDN 分发显示短内容/404，需稍候重试；curl 长输出用 `-o 文件` 方式读，避免 PowerShell 管道截断误报。
