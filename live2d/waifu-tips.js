...
!function () {
  ...
  function safeTipContent(msg) {
    const template = document.createElement("template");
    template.innerHTML = String(msg);
    template.content.querySelectorAll("*").forEach((node) => {
      if (!["BR", "SPAN"].includes(node.tagName)) {
  ...
  function showMessage(msg, duration, _priority) {
    ...
    el.replaceChildren(safeTipContent(msg));
  ...
  class ModelLoader {
    ...
    async loadRandModel() {
      ...
      if (this.useCDN) {
        if (!this.modelList) await this.loadModelList();
        const variants = this.modelList.models[id] || [];
        if (variants.length <= 1) {
          showMessage("当前人物只有一套服装，可以试试切换人物。", 4000, 10);
          return;
        }
        const next = (Number(tid) + 1) % variants.length;
        this.loadModel(id, next, "已切换到下一套服装。");
      } else {
  ...
  const tools = {
    ...
    asteroids: {
      icon: SVG.asteroids,
      callback: () => {
        if (document.body.classList.contains("ASTEROIDSYEAH")) {
          showMessage("飞船系统已经在运行，请使用方向键和空格操作。", 4000, 9);
          return;
        }
        if (window.Asteroids) {
          ...
          if (document.querySelector('script[data-qiguang-asteroids]')) return;
          const s = document.createElement("script");
          s.src = "https://cdn.jsdelivr.net/gh/TianXiaoQiang0621/qiguangji@main/qiguangji/public/live2d/asteroids.js?v=3";
          s.dataset.qiguangAsteroids = "true";
          s.onerror = () => { s.remove(); showMessage("飞船系统启动失败，请刷新后重试。", 4000, 9); };
  ...
  function init(config) {
    ...
    const labels = { chat: "聊天室", hitokoto: "随机一言", asteroids: "飞船小游戏", "switch-model": "切换人物", "switch-texture": "切换服装", photo: "人物截图", info: "后台管理", quit: "收起人物" };
    ...
    for (const name of toolList) {
      ...
      document.getElementById("waifu-tool").insertAdjacentHTML("beforeend",
        `<button type="button" id="waifu-tool-${name}" title="${labels[name] || name}" aria-label="${labels[name] || name}">${icon}</button>`
      );
  ...
  window.initWidget = function (opts, apiPath) {
    ...
    document.body.insertAdjacentHTML("beforeend",
      '<button type="button" id="waifu-toggle" aria-label="展开悬浮人物"><span>展开人物</span></button>'
    );
