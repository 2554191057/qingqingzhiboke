...
(function () {
  ...
  var CSS = [
    ...
    '.qw-panel{width:620px;max-width:95vw;max-height:calc(100dvh - 40px);min-width:380px;min-height:500px;height:720px;display:flex;flex-direction:column;background:var(--jp-surface);border:1px solid var(--jp-line);border-radius:18px;box-shadow:0 28px 100px rgba(0,0,0,.4),0 0 40px var(--jp-glow);overflow:hidden;color:var(--jp-ink);animation:qwPop .3s cubic-bezier(.16,1,.3,1);resize:none;}',
'.qw-resize-handle{position:absolute;right:0;bottom:0;width:15px;height:15px;cursor:nwse-resize;z-index:10;background:linear-gradient(135deg,transparent 50%,var(--jp-muted) 50%,var(--jp-muted) 60%,transparent 60%,transparent 70%,var(--jp-muted) 70%,var(--jp-muted) 80%,transparent 80%,transparent 90%,var(--jp-muted) 90%,var(--jp-muted) 100%);opacity:.5;border-bottom-right-radius:18px;}',
